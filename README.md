...
* AGNTCY-issued W3C Verifiable Credentials for portable capability attestation 
* IETF Identity Assertion JWT Authorization Grant (ID-JAG, Okta XAA) for IdP-mediated cross-domain token exchange per RFC 8693 
* [TBD] Cedar policy engine(policy evaluation and definition point) for fine-grained, declarative cross-boundary, task based agent authorization, with a credential minter to generate ephemeral creds for spawned OBO(on behalf of) agents. 
* End-to-end subject propagation for cryptographically auditable delegation chains.(Will need a workaround if 'sub' field can't be used for context) 
